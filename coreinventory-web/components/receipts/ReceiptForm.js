...
export default function ReceiptForm({ initialData, onSubmit, isLoading, isEdit = false }) {
  ...
  const calculateTotal = () => {
    return formData.items.reduce((sum, item) => 
      sum + (Number(item.expectedQuantity) * Number(item.unitCost)), 0
    ).toFixed(2);
  };

  const calculateTotalQuantity = () => {
    return formData.items.reduce((sum, item) => 
      sum + Number(item.expectedQuantity), 0
    );
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-[#EDE9FE] p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            <label className="block text-sm font-medium text-[#374151]">Expected Delivery Date</label>
            <div className="relative">
