...
import { Eye, Edit, Trash2, CheckCircle, XCircle, Package, Calendar, User, Truck } from 'lucide-react';
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-[#F9F7FF] border-b border-[#EDE9FE]">
            <tr>
              <th className="text-left py-4 px-6 text-xs font-semibold text-[#6B7280] uppercase tracking-wider">Delivery #</th>
              <th className="text-left py-4 px-6 text-xs font-semibold text-[#6B7280] uppercase tracking-wider">Customer</th>
          ...
          <tbody className="divide-y divide-[#F3F0FF]">
            {deliveries.map((delivery) => (
              <tr key={delivery._id} className="hover:bg-[#F9F7FF] transition-colors">
                <td className="py-4 px-6">
                  <Link 
                    href={`/operations/deliveries/${delivery._id}`}
                ...
                <td className="py-4 px-6">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-[#1a1a2e]">{delivery.customer?.name}</span>
                  </div>
                </td>
                <td className="py-4 px-6">
                  <span className="text-sm text-[#374151]">{delivery.warehouseId?.name}</span>
                </td>
                <td className="py-4 px-6">
                  <div className="flex items-center gap-2">
                    <Calendar size={14} className="text-[#9CA3AF]" />
                    <span className="text-sm text-[#374151]">{formatDate(delivery.deliveryDate)}</span>
                  </div>
                </td>
                <td className="py-4 px-6">
                ...
                <td className="py-4 px-6 text-right">
                  <span className="text-sm font-medium text-[#1a1a2e]">{delivery.items?.length || 0}</span>
                </td>
                <td className="py-4 px-6 text-right">
                  <div>
                    <span className="text-sm font-semibold text-[#1a1a2e]">{delivery.totalOrderedQuantity || 0}</span>
                    {delivery.totalDeliveredQuantity > 0 && (
                      <span className="text-xs text-[#6B7280] ml-1">
                        (del: {delivery.totalDeliveredQuantity})
                      </span>
                    )}
                  </div>
                </td>
                <td className="py-4 px-6">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    {delivery.status === 'Draft' && (
                      <>
                        <Link
                          href={`/operations/deliveries/${delivery._id}/edit`}
