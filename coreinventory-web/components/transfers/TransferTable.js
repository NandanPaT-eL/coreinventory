...
import { Eye, Edit, CheckCircle, XCircle, Package, Calendar, ArrowLeftRight } from 'lucide-react';
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-[#F9F7FF] border-b border-[#EDE9FE]">
            <tr>
              <th className="text-left py-4 px-6 text-xs font-semibold text-[#6B7280] uppercase tracking-wider">Transfer #</th>
              <th className="text-left py-4 px-6 text-xs font-semibold text-[#6B7280] uppercase tracking-wider">From → To</th>
              ...
              <th className="text-right py-4 px-6 text-xs font-semibold text-[#6B7280] uppercase tracking-wider">Total Qty</th>
          ...
          <tbody className="divide-y divide-[#F3F0FF]">
            {transfers.map((transfer) => (
              <tr key={transfer._id} className="hover:bg-[#F9F7FF] transition-colors">
                <td className="py-4 px-6">
                  <Link 
                    href={`/operations/transfers/${transfer._id}`}
                ...
                <td className="py-4 px-6">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-[#1a1a2e]">{transfer.fromWarehouse?.name}</span>
                    <ArrowLeftRight size={14} className="text-[#9CA3AF]" />
                    <span className="text-sm font-medium text-[#1a1a2e]">{transfer.toWarehouse?.name}</span>
                  </div>
                </td>
                <td className="py-4 px-6">
                  <div className="flex items-center gap-2">
                    <Calendar size={14} className="text-[#9CA3AF]" />
                    <span className="text-sm text-[#374151]">{formatDate(transfer.transferDate)}</span>
                  </div>
                </td>
                <td className="py-4 px-6">
                ...
                <td className="py-4 px-6 text-right">
                  <span className="text-sm font-medium text-[#1a1a2e]">{transfer.items?.length || 0}</span>
                </td>
                <td className="py-4 px-6 text-right">
                ...
                <td className="py-4 px-6">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    {transfer.status === 'Draft' && (
                      <>
                        <Link
                          href={`/operations/transfers/${transfer._id}/edit`}
